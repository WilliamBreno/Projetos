...
while True:
    ...
    if event == sg.WINDOW_CLOSED:
        ...
        if not rows:
            sg.popup('A lista está vazia. Não há registros para deletar.')
        else:
        ...
        confirm = sg.popup_yes_no(
            'Tem certeza que deseja deletar todos os dados da tabela?')
        if confirm == 'Yes':
    ...
    if event == 'Deletar Específico':
        ...
        window_popup = sg.Window('Deletar personagem:', layout_popup)
        while True:
